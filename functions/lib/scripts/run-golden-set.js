...
async function main() {
    ...
    const goldenSet = JSON.parse(content);
    const results = [];
...
async function askQuestion(baseUrl, message) {
...
function evaluate(q, answer, durationMs) {
    const errors = [];
    // 1. Teve resposta?
    const hasReply = typeof answer.reply === 'string' && answer.reply.length > 0;
    if (!hasReply)
        errors.push('sem resposta');
    // 2. Refusou como esperado?
    let refusedAsExpected = null;
    if (q.shouldRefuse) {
        const refused = !answer.inScope || answer.guardrailTriggered !== undefined;
        refusedAsExpected = refused;
        if (!refused) {
            errors.push(`deveria ter recusado (inScope=${answer.inScope}, guardrail=${answer.guardrailTriggered})`);
        }
            ...
            if (answer.guardrailTriggered !== q.expectedRefusalReason) {
                ...
                const accepted = acceptable[q.expectedRefusalReason] || [];
                if (!accepted.includes(answer.guardrailTriggered)) {
                    errors.push(`recusou com motivo "${answer.guardrailTriggered}", esperado "${q.expectedRefusalReason}"`);
                }
            }
        }
    }
        ...
        if (!answer.inScope)
            errors.push('deveria ter respondido, mas recusou');
    }
    ...
    if (q.expectedKeywords && q.expectedKeywords.length > 0 && !q.shouldRefuse) {
        ...
        if (!hasKeywords)
            errors.push(`nenhuma das keywords apareceu: ${q.expectedKeywords.join(', ')}`);
    }
    // 4. Citou a fonte esperada?
    let hasCitation = null;
    if (q.expectedCitation && !q.shouldRefuse) {
        hasCitation = (answer.sources || []).some((s) => s.docId === q.expectedCitation);
        if (!hasCitation) {
            errors.push(`não citou ${q.expectedCitation} (citou: ${(answer.sources || []).map((s) => s.docId).join(', ') || 'nenhuma'})`);
        }
    }
...
//# sourceMappingURL=run-golden-set.js.map