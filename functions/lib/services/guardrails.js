...
exports.checkScopeGuardrail = checkScopeGuardrail;
...
function checkScopeGuardrail(query, chunks) {
    // 1. Jailbreak
    for (const p of JAILBREAK_PATTERNS) {
        if (p.test(query)) {
            return {
                inScope: false,
                reason: 'jailbreak',
                refusalMessage: 'Desculpe, não posso processar essa solicitação. Posso ajudá-lo(a) com consultas sobre o material do CAOCIPP.',
            };
        }
    }
    ...
    for (const p of CASE_ANALYSIS_PATTERNS) {
        if (p.test(query)) {
            return {
                ...
                refusalMessage: 'Este assistente não analisa casos concretos — o exame do acervo fático-probatório é de responsabilidade do órgão de execução (Ordem de Serviço nº 002/2015, art. 2º, IX).\n\nPosso ajudá-lo(a) a:\n- Localizar material institucional sobre a tese jurídica aplicável\n- Abrir uma consulta formal ao CAOCIPP para análise específica',
            };
        }
    }
    // 3. Off-topic
    ...
    if (chunks.length === 0) {
        return {
            inScope: false,
            reason: 'no_results',
            refusalMessage: 'Não encontrei material específico sobre isso no acervo do CAOCIPP. Posso orientá-lo(a) na abertura de uma consulta formal ao CAOCIPP?',
        };
    }
