/**
 * Detecção de perguntas sobre o próprio agente.
 * Se detectado, retorna resposta canônica do bloco # IDENTIDADE.
 *
 * Anti-UX-bug: bot NUNCA deve dizer "não encontrei material sobre mim".
 * Inspirado no padrão do Lexio (orchestrator handles identity early).
 */
...
// Padroes de perguntas sobre o agente (case-insensitive, busca em texto normalizado sem acento).
// Importante: \b nao funciona com acentos em JS, por isso normalizamos antes.
const SELF_PATTERNS: RegExp[] = [
  ...
  /\bme\s+ajud[ae]?\b/i, // qualquer "me ajuda"
  ...
  /\bcofrito\b/i, // qualquer menção ao nome
]

/** Normaliza texto removendo acentos (para que \b funcione em JS) */
...
export function isAboutItself(question: string): boolean {
  if (!question) return false
  const q = question.trim()
  // Se a pergunta é longa (>200 chars) e contém termos jurídicos, provavelmente NÃO é sobre o agente
...
export function getIdentityResponse(short = false): string {
